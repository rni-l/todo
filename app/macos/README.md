...
应用仅在 `127.0.0.1:38889` 监听。菜单栏显示逾期、今天及未来 7 天的任务，支持添加今天任务和完成任务。小组件在服务停止时保留最近一次成功读取的内容，并标注为旧数据；系统决定小组件的定时刷新时机。构建时生成的快捷入口密钥放在应用和扩展各自的资源包里；若主窗口仍保有有效登录态，重新打开主窗口即可恢复菜单栏权限。
